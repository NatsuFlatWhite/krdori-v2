...
class ThreadingWSGIServer(ThreadingMixIn, WSGIServer):
    """Keep an idle SDK/WebView connection from blocking every request."""
        ...
        response = json.dumps(
            {
                'nickName': 'KRdori',
                'profileImageURL': (
                    'https://bandori-assets.rainbowunicorn7297.com/kr/'
                    'img_640x640.jpg'
                ),
                'thumbnailURL': (
                    'https://bandori-assets.rainbowunicorn7297.com/kr/'
...
def start(port):
    with make_server(
            '', port, application, server_class=ThreadingWSGIServer) as httpd:
